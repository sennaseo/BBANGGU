...
export function PackageSelect({ onConfirm, bakeryData }: PackageSelectProps) {
  ...
  const handleIncrement = () => {
    if (quantity < packageData.quantity) {
      // 남은 수량보다 작을 때만 증가
      ...
      <div className="flex-1 px-5">
        ...
        <div className="bg-white border border-[#F2F2F2] shadow-md rounded-xl p-5 mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-[18px] font-bold mb-2 text-[#333333]">
                {packageData.name}
              </h3>
              <div className="flex flex-col gap-2">
                <span className="text-[15px] text-[#A9A9A9]">
                  남은 수량:{" "}
                  <span className="font-bold">{packageData.quantity}개</span>
                </span>
                <div className="flex items-center gap-1">
                  <span className="text-[18px] font-bold text-[#333333]">
                    {packageData.price.toLocaleString()}원
                  </span>
                  <span className="text-[14px] text-[#E1E1E1] line-through">
                    {(packageData.price * 2).toLocaleString()}원
                  </span>
                </div>
              </div>
            </div>
        ...
        <div className="bg-white rounded-xl p-5 border border-[#F2F2F2] shadow-md">
          ...
          <div className="flex justify-between items-center pt-4 border-t border-gray-100">
            ...
            <span className="text-[18px] font-bold">
              {(packageData.price * quantity).toLocaleString()}원
            </span>
          </div>
        </div>
      </div>

      <PaymentButton onClick={handleConfirm} text={`${quantity}개 구매하기`} />
    </div>
  );
}
